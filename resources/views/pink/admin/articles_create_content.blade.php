...
{!! Form::open(['url' => (isset($article->id)) ? route('articles_panel.update',['articles_panel'=>$article->alias]) : route('articles_panel.store'),'class'=>'contact-form','method'=>'POST','enctype'=>'multipart/form-data']) !!}
	...
	<ul>
		<li class="text-field">
		 ...
		 <li class="text-field">
			...
			<div class="input-prepend"><span class="add-on"><i class="icon-user"></i></span>
			{!! Form::text('alias', isset($article->alias) ? $article->alias  : old('alias'), ['placeholder'=>'Введите псевдоним, например - article-10']) !!}
			 </div>
		 </li>
		...
		@if(isset($article->img->path))
			<li class="textarea-field">
				...
				{{ Html::image(asset('pink').'/images/articles/'.$article->img->path,'',['style'=>'width:400px']) }}
